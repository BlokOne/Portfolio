...
const fetchData = async () => {
   try {
      ...
      const result = await fetch(`https://api.openweathermap.org/data/2.5/weather?q=${query}&lang=ru&appid=010ca72c1f788c05712d5b4a83574531&units=metric`);     
      const data = await result.json();
      const timeNow = await correctTime (data.timezone);
      const { 
         timezone,
         visibility,
         name,
...
const renderProperty = (properties) => {
   return Object.values(properties).map(({title,value,icon}) => {
      ...
      <div class="property-icon">
         <img src="app/weather/image/icons/${icon}" alt="">
      </div>
...
const markup = () => {
   ...
   const containerClass = onlyHours < 18 && onlyHours > 7 ? 'is-day' : "";
   return `
         <div class="container ${containerClass}">
            <div class="top">
               ...
               <div class="city-info">
                  <div class="top-left">
                     <img src="app/weather/image/${getImage(description)}" alt="" class="icon">
